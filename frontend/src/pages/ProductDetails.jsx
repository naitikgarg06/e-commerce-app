...
export default function ProductDetails() {
  ...
  const { data } = useFetch(
    `https://e-commerce-app-ten-pi.vercel.app/products/${id}`
  );
  ...
  const [quantity, setQuantity] = useState(
    cart.filter((item) => item.productId._id === id)[0]?.quantity || 0
  );
  const [isInWishlist, setIsInWishlist] = useState(
    wishlist.includes(productDetails)
  );
                    ...
                    <div className="d-flex flex-column me-3">
                      ...
                      <div className="w-100">
                        <button
                          className="w-100"
                          onClick={async () => {
                            const isAdded = await addToCartHandler(
                              productDetails
                            );
                    ...
                    <div className="container py-3 d-flex flex-column flex-nowrap">
                      ...
                      <div className="d-flex flex-wrap">
                        ...
                        <button
                          ...
                          onClick={async () => {
                            if (quantity > 0) {
                              const cartId = cart.filter(
                                (item) =>
                                  item.productId._id === productDetails._id
                              )[0]._id;
                              const newQuantity = await decrementQuantity(
                                cartId,
                                quantity
                              );
                        ...
                        <button
                          ...
                          onClick={async () => {
                            if (quantity > 0) {
                              ...
                              const newQuantity = await incrementQuantity(
                                cartId,
                                quantity
                              );
                              setQuantity(newQuantity);
